...
export class InquilinoService{
    ...
    public getTodos(page:number,size:number):Observable<Page<Pessoa>>{
        return this.http.get<Page<Pessoa>>(`${this.pessoaService.local_route}/inquilinos?page=${page}&size=${size}`);
    }

    public getPorNome(nome:string,page:number,size:number):Observable<Page<Pessoa>>{
        return this.http.get<Page<Pessoa>>(`${this.pessoaService.local_route}/inquilino?nome=${nome}&page=${page}&size=${size}`);
    }
